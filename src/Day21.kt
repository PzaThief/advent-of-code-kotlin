...
private fun checkExample1() {
    ...
    val expected = 16L
    check(Day21Map(example).countPossibleGarden(6) == expected)
}
...
private class Day21Map(val map: List<String>) {
    ...
    fun countPossibleGarden(steps: Int, repeat: Boolean = false): Long {
        ...
        if (step == steps) return countsByStep[step]!!

        val y1 = countsByStep[mod]!!
        val y2 = countsByStep[mod + map.size]!!
        val y3 = countsByStep[mod + map.size * 2]!!
